...
export async function getSoldColor() {
  ...
  for (let i = 0; i < color.length; i++) {
    const [total] = await pool.query(
      "SELECT SUM (qty) FROM order_lists Where color_code=?",
      [color[i].color_code]
    );
...
export async function getSoldPrice() {
  ...
  for (let i = 500; i < 2000; i += 20) {
    [price] = await pool.query(
      "SELECT SUM (qty) FROM order_lists WHERE price BETWEEN ? AND ?",
      [i, i + 19]
    );
...
export async function getTopFive() {
  ...
  [candidates] = await pool.query(
    "SELECT DISTINCT product_id FROM order_lists;",
    []
  );
  for (let i = 0; i < candidates.length; i++) {
    const [total] = await pool.query(
      "SELECT SUM (qty) FROM order_lists Where product_id=?",
  ...
  for (let i = 0; i < topFive.length; i++) {
    ...
    for (let j = 0; j < 3; j++) {
        ...
        if (j==1){size='M'}
        if (j==2){size='L'}
      const [topFiveTotal] = await pool.query(
        "SELECT SUM (qty) FROM order_lists Where product_id=? AND size=?",
        [topFive[i],size]
      );
