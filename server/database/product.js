...
//---------------Create Product----------------------------------------
export async function createProduct(category, title, description, price, texture, wash, place, note, story, variants, main_image, images) {
    try {
        ...
        VALUES(?,?,?,?,?);
        `,
                [productResult.insertId, variants[i].color_name, variants[i].color_code, variants[i].size, variants[i].stock]
            );
        }
...
//---------------Product List API--------------------------------------
function addUrlForMainImage(arr) {
    arr.map((element) => (element.main_image = `${workingUrl}${element.main_image}`));
    return arr;
}
...
async function addUrlForImages(arr) {
    let urlImages = arr.map((element) => `${workingUrl}${element}`);
    return urlImages;
}
...
async function getImages(id) {
    ...
    let urlImages = await addUrlForImages(images);
    return urlImages;
}
...
export async function getProduct(category, paging) {
    ...
    products = await addUrlForMainImage(products);
    let final = [];
...
export async function getProductSearch(keyword, paging) {
    ...
    rows = await addUrlForMainImage(rows);
    let final = [];
